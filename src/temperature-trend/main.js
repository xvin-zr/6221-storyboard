...
function drawChart(processedData) {
    ...
    const yMin = Math.min(d3.min(processedData, d => d.lower), d3.min(processedData, d => d.mean));
    const yMax = Math.max(d3.max(processedData, d => d.upper), d3.max(processedData, d => d.mean));
    const yPadding = (yMax - yMin) * 0.1; // 添加10%的边距
    ...
    // upper
    const upperLine = svg.append("path")
    .datum(processedData)
    .attr("fill", "none")
    .attr("stroke", "rgba(188,143,143,0.8)")
    .attr("stroke-width", 1.5)
    .attr("d", d3.line()
        .x(d => x(d.date))
        .y(d => y(d.upper))
    ...
    // lower
    const lowerLine = svg.append("path")
    .datum(processedData)
    .attr("fill", "none")
    .attr("stroke", "rgba(188,143,143,0.8)")
    .attr("stroke-width", 1.5)
    ...
    .style("fill", "grey")           
    .text("Global Temperature Deviation from Baseline")
    ...
    let i = bisectDate(processedData, x0, 1);
    let d = x0 - processedData[i - 1].date > processedData[i].date - x0 ? processedData[i] : processedData[i - 1];
    ...
    tooltip.select(".tooltip-year")
        .text(`Year: ${formatYear(d.date)}`);
    tooltip.select(".tooltip-mean")
        .text(`Mean: ${d.mean.toFixed(2)} °C`);
    tooltip.select(".tooltip-lower")
        .text(`Lower: ${d.lower.toFixed(2)} °C`);
    tooltip.select(".tooltip-upper")
        .text(`Upper: ${d.upper.toFixed(2)} °C`);
    ...
    function bisectDate(array, x, lo = 0, hi = array.length) {
    while (lo < hi) {
        const mid = lo + Math.floor((hi - lo) / 2);
        if (array[mid].date < x) lo = mid + 1; else hi = mid;
    }
    return lo;
    }
...
slider.addEventListener('mousedown', function(e) {
    const startThumbRect = startThumb.getBoundingClientRect();
    const endThumbRect = endThumb.getBoundingClientRect();
